...
import { TextField, Button, Box, Typography, List, ListItem, ListItemText, ListItemSecondaryAction, IconButton, Paper, Container } from '@mui/material';
...
const AddClasses = () => {
    ...
    const fetchDomainsAndClasses = async () => {
        try {
            const domainResponse = await fetch('http://localhost:5000/api/domain/getAll', {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            });
            const classResponse = await fetch('http://localhost:5000/api/class/getAll', {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            });
    ...
    const deleteDomain = async (domainId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/domain/delete/${domainId}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            });
    ...
    const deleteClass = async (classId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/class/delete/${classId}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            });
    ...
    return (
        <Container maxWidth="md">
            <Box padding={3} component={Paper}>
                <Typography variant="h4" gutterBottom>Manage Domains</Typography>
                <Box component="form" onSubmit={(e) => e.preventDefault()} marginY={2}>
                    <TextField
                        ...
                        variant="outlined"
                    />
                    <Button onClick={addDomain} variant="contained" color="primary" disabled={!newDomainOnlyName} sx={{ mt: 2 }}>
                        Add Domain
                    </Button>
                </Box>
                ...
                <Box component="form" onSubmit={(e) => e.preventDefault()} marginY={2}>
                    ...
                    <TextField
                        ...
                        variant="outlined"
                    />
                    <Button onClick={addClass} variant="contained" color="primary" disabled={!newClassName || !newDomainName} sx={{ mt: 2 }}>
                        Add Class
                    </Button>
                </Box>
                {error && <Typography color="error">{error}</Typography>}
                {domains.map((domain) => (
                    <Box key={domain._id} marginBottom={2} component={Paper} padding={2} mt={2}>
                        <Typography variant="h6">{domain.name}</Typography>
                        <List>
                        {domain.classes.map((cls) => (
                                <ListItem key={cls._id} divider sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <ListItemText primary={cls.name} />
                                    <ListItemSecondaryAction>
                                        <IconButton edge="end" aria-label="delete" onClick={() => deleteClass(cls._id)}>
                                            <DeleteIcon />
                                        </IconButton>
                                    </ListItemSecondaryAction>
                                </ListItem>
                            ...
                            <ListItem divider sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                <ListItemSecondaryAction>
                                    <IconButton edge="end" aria-label="delete" onClick={() => deleteDomain(domain._id)} disabled={domain.classes.length > 0}>
                                        <DeleteIcon />
                                    </IconButton>
                                </ListItemSecondaryAction>
                            </ListItem>
